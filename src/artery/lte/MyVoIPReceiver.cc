...
void MyVoIPReceiver::handleMessage(cMessage *msg)
{
    ...
    // EV << "MyVoIPReceiver::handleMessage - Packet received: TALK[" << pPacket->getIDtalk() << "] - FRAME[" << pPacket->getIDframe() << " size: " << (int)pPacket->getByteLength() << " bytes]\n";
    ...
    //mPacketsList_.push_back(check_and_cast<VoipPacket*>(pPacket));
}

void MyVoIPReceiver::playout(bool finish)
{
    ...
    while (!mPacketsList_.empty() /*&& pPacket->getIDtalk() == mCurrentTalkspurt*/)
    {
        ...
        if(!finish)
            EV << "MyVoIPReceiver::playout - Jitter measured: " << last_jitter << " TALK[" << pPacket->getIDtalk() << "] - FRAME[" << pPacket->getIDframe() << "]\n";
        ...
        if (isArrived[pPacket->getIDframe()])
        {
            // avoid printing during finish (as it will print to the standard output)
            if(!finish)
                EV << "MyVoIPReceiver::playout - Duplicated Packet: TALK[" << pPacket->getIDtalk() << "] - FRAME[" << pPacket->getIDframe() << "]\n";
            delete pPacket;
        }
        else if( last_jitter > 0.0 )
        {
            ...
            // avoid printing during finish (as it will print to the standard output)
            if(!finish)
                EV << "MyVoIPReceiver::playout - out of time packet deleted: TALK[" << pPacket->getIDtalk() << "] - FRAME[" << pPacket->getIDframe() << "]\n";
        ...
        else
        {
            ...
            if(mBufferSpace_ > 0)
            {
                ...
                if(!finish)
                {
                    EV << "MyVoIPReceiver::playout - Sampleable packet inserted into buffer: TALK["<< pPacket->getIDtalk() << "] - FRAME[" << pPacket->getIDframe()
            ...
            else
            {
                ...
                if(!finish)
                {
                    EV << "MyVoIPReceiver::playout - Buffer is full, discarding packet: TALK[" << pPacket->getIDtalk() << "] - FRAME["
    ...
    if(!finish)
    {
        EV << "MyVoIPReceiver::playout - proportionalLoss " << proportionalLoss << "(tailDropLoss=" << tailDropLoss << " - playoutLoss="
...
//    sample = SIMmPlayoutDelay_;
    ...
    if(!finish)
    {
        EV << "MyVoIPReceiver::playout - Computed MOS: eModel( " << mPlayoutDelay_ << " , " << tailDropLoss << "+" << playoutLoss << "+"
           << channelLoss << " ) = " << mos << "\n";

        EV << "MyVoIPReceiver::playout - Playout Delay Adaptation \n" << "\t Old Playout Delay: " << mPlayoutDelay_ << "\n\t Max Jitter Measured: "
           << max_jitter << "\n\n";
    }
...
double MyVoIPReceiver::eModel(simtime_t delay, double loss)
{
